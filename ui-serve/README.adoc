...
----
$ git clone git@github.com:${loggedUser}/${artifactId}
...
. Be sure that view access rights for the service account are added before deploying your boostoc policy add-role-to-user view -n $(oc project -q) -z defaulter using: `oc policy add-role-to-user view -n $(oc project -q) -z default`.
